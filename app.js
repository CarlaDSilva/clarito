...
// Identificador estable de producto: nace aquí, viaja por el Gist y LADESPENSA lo usa
// para no perder el vínculo cuando se renombra un producto.
const _pid=()=>(crypto.randomUUID?crypto.randomUUID():'p_'+Date.now().toString(36)+Math.random().toString(36).slice(2,10));
...
function migratePids(){
  ...
  for(const [key,v] of Object.entries(prods)){
    if(!v)continue;
    const g=normalizeKey(v.alias||key);
    (groups[g]||(groups[g]=[])).push([key,v]);
  }
  let changed=false;
  for(const list of Object.values(groups)){
    let pid=list.map(([,v])=>v.pid).find(Boolean)||_pid();
...
function renameKnowledgeProduct(key,newName){if(!newName.trim())return;const entry=DB.knowledge.products[key];if(!entry)return;if(!entry.pid)entry.pid=_pid();const trimmed=newName.trim();entry.alias=trimmed;const newKey=normalizeKey(trimmed);if(newKey&&newKey!==key)DB.knowledge.products[newKey]={...entry,alias:trimmed};}
...
const GistSync = {
  ...
  async pull(){
    ...
    if(DB.gistToken) headers['Authorization']='Bearer '+DB.gistToken;
    try{
